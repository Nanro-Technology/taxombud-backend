    ...
    public static async Task<IApplicationBuilder> SeedDatabaseAsync(this IApplicationBuilder app)
    {
        ...
        try
        {
            ...
            if (dbProvider != null && dbProvider.Contains("MySql"))
            {
                ...
                // NOTE: Workflow engine tables and stage pipeline tables are now managed by the
                // EF Core migration '20260810210829_AddWorkflowAndStageTables'. Do NOT add raw DDL
                // for those tables here — it causes 'Duplicate column/table' errors on startup.
            }
            else
            {
